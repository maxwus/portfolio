...
for dic in datasets:
    if (dic.get('measurement') == typeM) and  (dic.get('temperature') == temperature) and (dic.get('wire') == wire) and dic.get('magnet_drive') == magnet_drive:
        B = MagField(float(dic.get('magnet_drive')))             
        drive = dic.get('drive')
        I = ElCurrent(drive, resistance, attenuation)
        F = DrivingForce(B, wire_length, I)
        temp = (drive, F)
        drive_force.append(temp)
        data = dic.get('data')[:,2]
        temp = (drive, FindAmplitude(data))
